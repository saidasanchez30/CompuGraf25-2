...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	*/
	...
	if (key == GLFW_KEY_H)
	{
		...
		theWindow->articulacion3 = glm::clamp(theWindow->articulacion3, 0.0f, 40.0f);
	}
	if (key == GLFW_KEY_J)
	{
		theWindow->articulacion3 -= 10.0;
	...
	*/
	...
	{
		theWindow->articulacion10 = 2.0;
	}
		else if (key == GLFW_KEY_O && action == GLFW_RELEASE)
		{
			theWindow->articulacion10 = 0.0;
		}
	...
	if (key == GLFW_KEY_Z)
	{
		theWindow->Luciernaga = true;
	}
	if (key == GLFW_KEY_X)
	{
		theWindow->Luciernaga = false;
	}
