...
int main()
{
	...
	CreateObjects();
	CreateShaders();
	CrearCubo(); //índice 7 de MesjList
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = modelaux;
		model = glm::translate(model, glm::vec3(-2.1f, 0.7f, 3.4f));   //+x, -z, +y
		float radio = 0.5f;
		model = glm::rotate(model, glm::radians(10.0f * sin(angulovaria * 0.10f)), glm::vec3(1.0f, 0.0f, 0.0f)); // Oscila en X
		color = glm::vec3(1.0f, 0.0f, 0.0f); // Rojo
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Cabeza1.RenderModel();

		//Cabeza 2 del dragon -  Movimiento espiral de Arquímedes
		model = modelaux;
		model = glm::translate(model, glm::vec3(-1.8f, -0.3f, 3.5f));
		model = glm::rotate(model, glm::radians(angulovaria * 20.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		...
		model = glm::rotate(model, glm::radians(0.05f * angulovaria * angulovaria), glm::vec3(0.0f, 1.0f, 1.0f));
		...
		model = modelaux;
		model = modelaux2;
		model = glm::translate(model, glm::vec3(-0.8f, 0.8f, 3.2f));
		model = glm::rotate(model, glm::radians(30.0f * sin(angulovaria * 0.25f)), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		alaDer.RenderModel();

		//Ala Izquierda del dragon
		model = modelaux;
		model = glm::translate(model, glm::vec3(-0.8f, -0.4f, 3.2f));
		model = glm::rotate(model, glm::radians(-30.0f * sin(angulovaria * 0.25f)), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		alaIzq.RenderModel();
		...
		*/
		...
		/*Este if es para :
			...
			2. no se slae del rango de 0 a 1 del texturizado
		*/
		...
		if (tiempoActual - tiempoUltimoCambio >= intervaloCambio) {
			...
			indiceNumero = (indiceNumero + 1) % 4;
			toffsetnumerocambiau = indiceNumero * 0.25f;
		}
		toffsetnumerov = 0.0f;
		...
		double tiempoActualTex = glfwGetTime();
		if (tiempoActualTex - tiempoUltimoCambio >= intervaloCambio) {
			tiempoUltimoCambio = tiempoActualTex;
			indiceNumero = (indiceNumero + 1) % 2;  // Alterna entre 0 y 1
		}
		toffsetnumerou = 0.0;
		toffsetnumerov = 0.0;
		toffset = glm::vec2(toffsetnumerou, toffsetnumerov);
